...
public class ConstantPool {
    ...
    public void read(InputStream inputStream) {
        ...
        for (int i = 1; i < constant_pool_count; i++) {
        	ConstantInfo cInfo = cpInfo[i];
        	if(cInfo instanceof ConstantNameAndType){
        		ConstantNameAndType c = (ConstantNameAndType)cInfo;
        		StringBuilder sb = new StringBuilder();
        		sb.append(((ConstantUtf8)cpInfo[c.nameIndex]).value);
        		sb.append(((ConstantUtf8)cpInfo[c.descIndex]).value);
        		c.setValue(sb.toString());
        	}else if(cInfo instanceof ConstantClass){
        		ConstantClass c = (ConstantClass)cInfo;
        		c.setValue(((ConstantUtf8)cpInfo[c.nameIndex]).value);
        	}
        }
        
        for (int i = 1; i < constant_pool_count; i++) {
        	ConstantInfo cInfo = cpInfo[i];
        	if(cInfo instanceof ConstantMemberRef){
        		ConstantMemberRef c = (ConstantMemberRef)cInfo;
        		StringBuilder sb = new StringBuilder();
        		sb.append((cpInfo[c.classIndex]).value);
        		sb.append((cpInfo[c.nameAndTypeIndex]).value);
        		c.setValue(sb.toString());
        	}
        }
    }
    	...
    	if(cp!=null){
    		    ...
    		    for(int i=1;i<cp.constant_pool_count;i++){
    		    	...
    		    	if(constantInfo instanceof ConstantMemberRef){
    		    		...
    		    		switch(tag){
    		    		    case 9:
    		    		    	System.out.println("#"+i+" fieldref:" + memberRef.value);
    		    		    	continue;
    		    		    case 10:
    		    		    	System.out.println("#"+i+" methodref:" + memberRef.value);
    		    		    	continue;
    	 	                default : 
    		    		    	System.out.println("#"+i+" interface-methodref:" + memberRef.value);
    	 	                	continue;
    		    		}
